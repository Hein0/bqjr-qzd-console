var seekDays,
	currMonth,     //当前月份固定不变
	changeMonth,   //日历上面显示的月份 用于传给后台设置不同月份的资方方案
	currYear,      //当前年份
	tagDays = [];
//获取每月数据
function getData(csMonth){
    $.ajax({
        type: "post",
        url: base + 'cae/getCaptitalSeek.do',
        async: false,
        data: {csMonth: csMonth},
        dataType: "JSON",
        success: function (data) {
            if(data.status == 0) {
                seekDays = data.data.captitalSeekDayMap;
                for (var key in seekDays) {
                    tagDays.push(key);
                }
            }else{
			...
			for(var n=0;n<7;n++){//列
				if((7*m+n) < codeFirst || (7*m+n) >= (codeFirst+monthDays[codeMonth])){//某月日历中不存在的日期
					...
					if((7*m+n+1-codeFirst) >= codeDay){
                        if($.inArray((7*m+n+1-codeFirst).toString(), tagDays) != -1){
                            kalendar_html += "<li class='fileDiv'><span>"+(7*m+n+1-codeFirst)+"</span><p title='"+seekDays[7*m+n+1-codeFirst].dayRemark+"'>"+seekDays[7*m+n+1-codeFirst].dayRemark+"</p></li>";
                        }else{
                            kalendar_html += "<li class='fileDiv'><span>"+(7*m+n+1-codeFirst)+"</span></li>";
                        }
					}else{
                        if($.inArray((7*m+n+1-codeFirst).toString(), tagDays) != -1){
                            kalendar_html += "<li><span>"+(7*m+n+1-codeFirst)+"</span><p title='"+seekDays[7*m+n+1-codeFirst].dayRemark+"'>"+seekDays[7*m+n+1-codeFirst].dayRemark+"</p></li>";
                        }else{
                            kalendar_html += "<li><span>"+(7*m+n+1-codeFirst)+"</span></li>";
                        }
					}
				}
			}
	...
	function dateChange(dateObj,dateMonth,dateFirstDay,dateTotalDays,dateCurrentDay){
		...
		if(newLine > dateLine){//增加行
			...
			for(var i=dateLine-1;i>=newLine;i--){
				$('.dayListHide'+i).hide();
                $('.dayListHide'+i).find("li").removeClass();
			}
		}
		//重置日期排序
		dateLine = newLine;
		...
		*/
		...
		for(var i=0;i<7*newLine;i++){
			if(i < dateFirstDay || i> (dateTotalDays+dateFirstDay-1)){//日历中 当月不存在的日期
                ...
                if(yearChange > currYear || (yearChange == currYear && dateMonth > currMonth) || (yearChange == currYear && dateMonth == currMonth && (i+1-dateFirstDay) >= dateCurrentDay)){
                    dateObj.eq(i).addClass("fileDiv");
				}
                if($.inArray((i+1-dateFirstDay).toString(), tagDays) != -1){
                    dateObj.eq(i).html('<span>'+(i+1-dateFirstDay)+'</span><p title="'+seekDays[i+1-dateFirstDay].dayRemark+'">'+seekDays[i+1-dateFirstDay].dayRemark+'</p>');
                }else{
                    dateObj.eq(i).html('<span>'+(i+1-dateFirstDay)+'</span>');
                }
			}
		}
	}
