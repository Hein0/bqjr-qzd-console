...
var pageUtil = {
    ...
    'getSearchValue': function () {
        var data = {},
            $wrap = $('#pageSearchForm');
        data.province = $wrap.find(".select-citytal").attr("data-id") || "";
		data.type = 1;
        return data;
    },
    ...
    'rendershopSku': function (data) {
        ...
        $.each(data, function (childIndex, child) {
            ...
            html += '<tr class="text-c list-line" data-id="' + child.id + '" data-imgUrl="' + child.imgUrl + '" data-province="' + child.province + '" data-link="' + child.link + '" >\
		        ...
		        <td>' + (child.link || '') + '</td>\
		        <td class="f-14">\
		        	<a href="javascript:void(0)" class="bannerSort malr06 blues" target="">排序</a>\
		        	<a href="javascript:void(0)" class="bannerEdit malr06 blues" target="">编辑</a>\
		        	<a href="javascript:void(0)" class="bannerDelete malr06 blues" target="">删除</a>\
			    </td>\
		    </tr>';
        })
...
$(function () {
    ...
    CacheData.getData('getArea', function (data) {
        ...
        $.each(data, function (index, item) {
        	html += '<li data-id="' + item.province + '">' + item.province + "</li>";
        });
    ...
    $('#screeBtn').on('click', function () {
    	if($(".select-citytal").attr("data-id") !=""){
    ...
    $('#resetBtn').on('click', function () {
        ...
        $wrap.find(".select-citytal").attr("data-id", "").text("请选择地区");
...
//      pageUtil.getBannerList();
    ...
    // 商品列表相关事件绑定
    $('#listCtnWrap')
    	.on('click', '.bannerSort', function () {//排序
            var $self = $(this),
        ...
        .on('click', '.bannerEdit', function () {//编辑
            var $self = $(this),
                $parent = $self.closest('.list-line'),
                id = $parent.attr('data-id'),
            ...
            var goUrl = "shopModify.html?byId="+ id +"&imgUrl="+ imgUrl +"&province="+ province +"&links="+ links;
