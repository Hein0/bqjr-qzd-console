...
var withdraw = {
	...
	getOrderNub:function(){
		...
		$.ajax({
			type: "POST",
			url: base + "orderMag/employeeOrder/count",
...
//			util.reset();//清空筛选
		...
		$(".type-select-panel.city")
			...
			.on("click", ".get-result", function() {
				...
				var $self = $(this),
					...
					$target.text(withdraw.getSelectCityValue($self)).attr("data-id", $parent.find(".check:last").attr("data-id"));
					$parent.hide();
			});
	...
	queryList: function (obj) {
        ...
        $.ajax({
			type: "POST",
			url: base + "orderMag/employeeOrder",
			data: JSON.stringify(detParam),
			contentType: "application/json;charset=utf-8",
			success: function(res) {
				if (res.status == 0 && res.data) {
	...
	rendersTmpl:function($wrap,data){
        ...
        $.each(data, function (childIndex, child) {
        	var str = '<p>' + (child.tel || '') + '</p>';
        	(child.status  == 0)&& (str='') ;
            html += '<tr class="text-c list-line" data-id="' + child.id + '" >\
		        <td>' + (child.reserveOrderNo || '') + '</td>\
		        <td><p>' + (child.userName || '') + '</p>'+ str +'</td>\
		        <td>' + (child.goodId || '') + '</td>\
		        <td>' + (child.goodName || '') + '</td>\
		        <td><p>下单:' + (child.addTime || '') + '</p><p>付款:' + (child.payTime || '') + '</p></td>\
		        <td>' + (child.qzPrice || '') + '</td>\
		        <td>' + (child.amount || '') + '</td>\
		        <td>' + (child.status || '') + '</td>\
		        <td>' + (child.appoGold || '') + '</td>\
		        <td>' + (child.pendPay || '') + '</td>\
		    </tr>';
        })
        $wrap.html(html);
