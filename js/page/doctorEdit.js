	...
	init: function() {
		...
		if(path.indexOf("?") >= 0) {
			...
			function callBack() {
			        ...
			        if (spuUtils.doctorSpuData["img" + (index + 1)]) {
			            ...
			            $(domTemp).attr("data-src", spuUtils.doctorSpuData["img" + (index + 1)]).next().prepend(
			                '<img style="display: inline-block;width: 100px;height: 100px;" src="' +
			                  imgPath + spuUtils.doctorSpuData["img" + (index + 1)] + '">'
			            );
			        }
		        }):[];
				...
				$(".spu-cont .select-hospital").attr("data-id", spuUtils.doctorSpuData.hospital_id).html(spuUtils.doctorSpuData.hospital);
...
//	    if (flag) {
//	        var index = parent.layer.getFrameIndex(window.name);
//	        parent.layer.close(index ? index : spuUtils.doctorSpuData.index);
//	    } else {
			...
			$.ajax({
				...
				success: function(res) {
					if(res.message == "操作成功") {
						...
						var _data = res.data;
						var goUrl = "doctorNewDetails.html?";
