...
var pageUtil = {
    ...
    getLogsList: function(obj) {
        ...
        pageUtil.cache.pageInfo = {
         ...
         !data.hasOwnProperty('grade') && (data.grade = '');
        if(pageUtil.role.id == "10") {
            ...
            $.get(base + "case/getCaseBrowsing", data, function(res) {
                if(res.status == "0" && res.data) {
                    if(res.data.length) {
                        pageUtil.cache.listCache = res.data;
                        pageUtil.renderLogsSku(res.data);
    ...
    renderLogsSku: function(data) {
        ...
        $.each(data, function(childIndex, child) {
            ...
            html +=
                ...
                (child.city_name || "") +
                '</td>\
                    <td> <img class="avatar size-XL" src="' +
                (child.before_main ? imgPath + child.before_main : "") +
                '"></td><td><img class="avatar size-XL" src="' +
                                ...
                                child.status == 3 ?
                                    ...
                                    child.status == 4 ?
                                        ...
                                        child.status == 5 ?
                                            "审核成功" :
                                            child.status ? child.status : "") +
                '</td><td class="td-grade">'+ (child.grade = child.grade || "") +'</td><td class="td-scanNum">'+ child.scanNum +'</td><td class="td-voteNum">'+ child.voteNum +'</td><td class="td-evaluateNum">'+ child.evaluateNum +'</td>\
                    <td class="f-14"> <a href="javascript:void(0)" class="see-detail blues see" style="padding: 0 5px">查看</a><a href="javascript:void(0)" class="blues rate" style="padding: 0 5px">评级</a><span class="edit"></span>' + "</td></tr>";
        });
        $("#listCtnWrap").html(html);
    },

    // 浏览量配置表单提交
    submitBrowseData:function (index, $ele) {
        var param = $ele.find('form').serialize();

        layer.close(index);
        $.ajax({
            url: base + 'case/editCaseGrade',
            type: 'get',
            data: param,
    ...
    // 设置浏览量配置数据
    setBrowseData: function (obj) {

        var $parent = obj.parents('tr'), self = this;
        var arr = ['caseId','grade', 'scanNum', 'voteNum', 'evaluateNum'];

        layer.open({
            title: '浏览量配置',
            content: $('#browerConf').html(),
            area:'800px',
            success: function ($obj) {
                arr.forEach(function (t) {
                    $obj.find('*[name='+ t +']').val($parent.find('.td-' + t).html())
                });
            },
            yes: function (index, $ele) {
                self.submitBrowseData(index, $ele)
            }
        })
...
$(function() {
    ...
    // 浏览量配置弹窗
    $('#listCtnWrap').on('click', '.rate', function () {
        pageUtil.setBrowseData($(this))
    });
    ...
    // 相关联系人
        ...
        .on("click", ".see", function() {
            ...
            var toUrl = "../goods/logDetail.html?";
